#!/usr/bin/env bash
# B 档门禁：PyPTO dispatch 与 torch_npu 同进程共存实验。
set -uo pipefail
...
OUT=/data/sunkaixuan/skx_log_output/csa_b_tier/gate_$(date +%Y%m%d_%H%M%S)
...
cd $OUT/work || exit 1
$V/bin/python $D/pto_in_vllm_process.py --out-dir $OUT
EOF
